...
vs.\ 0.334 old-task retention). One honest caveat: on this rank-3 bench at
48 tasks the fixed encoder is again the best overall row---the learnable
conditions ran capacity-limited (576 total symbols against a 128-unit
dictionary cap), so the flagship learned-dictionary advantage of
Table~\ref{tab:longbench} should not be assumed to extend to regimes where
allocation demand far exceeds dictionary capacity.
...
ownership}: every bench was designed by us; there is no comparison against
EWC, replay, or progressive networks on benchmarks we do not control. A
pre-registered protocol for that comparison---including the interventional
criterion for what observed forgetting would count as \emph{outside} the
trichotomy, fixed before any external run---accompanies the code
(\code{paper/preregistration.md}).
